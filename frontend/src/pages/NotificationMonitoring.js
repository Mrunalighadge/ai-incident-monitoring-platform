...
function NotificationMonitoring() {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      const cpu = parseMetric("notification_cpu_usage");
      const memory = parseMetric("notification_memory_usage");
      const queue = parseMetric("email_queue");
      const failures = parseMetric("failed_notifications");
      const anomalies = parseMetric("notification_anomaly_score");

      const status =
        failures > 5 ? "CRITICAL" : queue > 70 ? "WARNING" : "HEALTHY";

      setStats({ cpu, memory, queue, failures, anomalies, status });

      const timestamp = new Date().toLocaleTimeString();
      setChartData((prev) => [
        ...prev.slice(-14),
        { time: timestamp, queue, failures },
      ]);
    } catch (error) {
  ...
  const getStatusColor = () => {
    if (stats.status === "CRITICAL") return "text-red-400";
    if (stats.status === "WARNING") return "text-yellow-400";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-10">
          <div>
            ...
            <h1 className="text-5xl font-bold mb-3">Notification Monitoring</h1>
            <p className="text-gray-400 text-lg">
              Queue analytics and delivery monitoring
            </p>
          </div>
          <div className="bg-slate-900 border border-red-500 rounded-2xl px-8 py-5">
            <p className="text-red-400 text-sm uppercase">Notification Status</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          ...
          <div className="bg-slate-900 rounded-3xl p-6">
            <p className="text-gray-400 mb-2">Email Queue</p>
            <h2 className="text-4xl font-bold text-yellow-400">
              {stats.queue.toFixed(0)}
            </h2>
          </div>
          <div className="bg-slate-900 rounded-3xl p-6">
            <p className="text-gray-400 mb-2">Failed Notifications</p>
            <h2 className="text-4xl font-bold text-red-400">
        ...
        <div className="bg-slate-900 rounded-3xl p-6 mb-10">
          ...
          <div className="h-[350px] bg-slate-800 rounded-2xl flex flex-col items-center justify-center">
            ...
            <p className="text-gray-400 text-center px-6 mb-6">
              View notification CPU usage, memory usage, alert traffic and
              delivery analytics in Grafana.
            </p>
            <a
              href="https://rubyplatypus1017.grafana.net/d-solo/mrmcbfb/notification-dashboard"
              target="_blank"
        ...
        <div className="bg-slate-900 rounded-3xl p-6 mb-10">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold">Notification AI Analysis</h2>
          ...
          <div className="h-[350px] bg-slate-800 rounded-2xl flex flex-col items-center justify-center">
            ...
            <p className="text-green-400">Notification service healthy</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-3xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-slate-800 rounded-2xl p-6">
              <p className="text-gray-400 mb-2">Predicted Issue</p>
              <h3 className="text-lg font-semibold">
                Queue backlog causing delayed processing
              </h3>
            </div>
            <div className="bg-slate-800 rounded-2xl p-6">
              <p className="text-gray-400 mb-2">Impact Analysis</p>
              <h3 className="text-lg font-semibold">
                Delayed email alerts and notification failures
              </h3>
            </div>
            <div className="bg-slate-800 rounded-2xl p-6">
              <p className="text-gray-400 mb-2">Recommended Action</p>
              <h3 className="text-lg font-semibold">
